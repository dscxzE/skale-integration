...
describe("AVVE", function () {
  ...
  async function deployOneYearLockFixture() {
    ...
    const vistiaPriceFeed = await ethers.getContractFactory("vistiaPriceFeed");
    const vistiaPriceFeed = await vistiaPriceFeed.deploy(
      18,
      "USDT/USD Price Feed"
    );
    await vistiaPriceFeed.waitForDeployment();

    const PriceOracleGetter = await ethers.getContractFactory(
      "PriceOracleGetter"
    );
    const priceOracleGetter = await PriceOracleGetter.deploy();

    await vistiaPriceFeed.transmit(
      "0x000000000000000000000000000000000000000000000000000000671229b806",
    ...
    priceOracleGetter.setAssetToPriceFeed(
      await mockUsdc.getAddress(),
      await vistiaPriceFeed.getAddress()
    );
  ...
  describe("Supply", function () {
    it("Test supply", async () => {
      const { pool, poolAddressesProvider, owner, mockUsdc, poolConfigurator } =
      ...
      const StableDebtToken = await ethers.getContractFactory(
        "StableDebtToken"
      );
      ...
      const variableDebtToken = await VariableDebtToken.deploy(
        ...
        "0x0000000000000000000000000000000000000000",
        6,
        "VariableDebtTokenUSDC",
        "VDTUSDC"
      );

      const ReserveInterestRateStrategy = await ethers.getContractFactory(
        "ReserveInterestRateStrategy"
      );
      const reserveInterestRateStrategy =
        await ReserveInterestRateStrategy.deploy(
          await pool.getAddress(),
      ...
      await poolAddressesProvider
        .connect(owner)
        .setPoolImpl(await pool.getAddress());
      await poolAddressesProvider.setPoolConfiguratorImpl(
        await poolConfigurator.getAddress()
      );

      await poolConfigurator.setReserveActive(
        await mockUsdc.getAddress(),
        true
      );
