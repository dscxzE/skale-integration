...
const AvistiaToken = {
  ...
  aTokenDecimals: 18,
  aTokenName: "vistia Ripple USDT",
  aTokenSymbol: "avistiaUSDT",
};
...
async function main() {
  ...
  // AvistiaToken.treasury = treasuryAddress

  // const StableDebt = await hre.ethers.getContractFactory("StableDebtToken")
  // const stableDebt = await StableDebt.deploy(AvistiaToken.pool, AvistiaToken.underlyingAsset, AvistiaToken.incentivesController, AvistiaToken.aTokenDecimals, "vistia Ripple USDT Stable Debt", "avistiaUSDTSDT")
  ...
  // const variableDebt = await VariableDebt.deploy(AvistiaToken.pool, AvistiaToken.underlyingAsset, AvistiaToken.incentivesController, AvistiaToken.aTokenDecimals, "vistia Ripple USDT Variable Debt", "avistiaUSDTVDT")
  ...
  // const interestRateStrategy = await InterestRateStrategy.deploy(AvistiaToken.addressProvider,
  //     optimalUsageRatio,
  ...
  // const aToken = await AToken.deploy(AvistiaToken.pool, AvistiaToken.treasury, AvistiaToken.underlyingAsset, AvistiaToken.incentivesController, AvistiaToken.aTokenDecimals, AvistiaToken.aTokenName, AvistiaToken.aTokenSymbol)
  // await aToken.waitForDeployment()
  // const atokenAddress = await aToken.getAddress()

  // const Pool = await hre.ethers.getContractAt("Pool", AvistiaToken.pool)
  const PoolConfigurator = await hre.ethers.getContractAt(
    "PoolConfigurator",
    "0x95E0e5f14Edd1a28ada89b0F686eAaF81Da91c37"
  );

  // const txPool = await Pool.connect(account).initReserve(
  //     AvistiaToken.underlyingAsset,
  //     atokenAddress,
  //     stableDebtAddress,
  //     variableDebtAddress,
  ...
  const txReserveActive = await PoolConfigurator.setReserveActive(
    AvistiaToken.underlyingAsset,
    true
  );
  await txReserveActive.wait();

  const txReserveBorrowing = await PoolConfigurator.setReserveBorrowing(
    AvistiaToken.underlyingAsset,
    true
  );
...
async function supply() {
  ...
  try {
    ...
    const tx = await Pool.connect(account).supply(
      "0xcD84fcd2964612D1585F1494B8Ed4F1Ae29D32AC",
